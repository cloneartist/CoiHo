...
class _ViewRecordState extends State<ViewRecord> {
  ...
  Widget build(BuildContext context) {
    Widget createTable(List<Track> track) {
      List<TableRow> rows = [];
      for (int i = 1; i < track.length; ++i) {
        rows.add(TableRow(children: [
          Container(
            child: Text("${track[i].time}"),
            color: Colors.red,
          ),
          Text("${track[i].temp}"),
          Text("${track[i].spO2}"),
          Text("${track[i].pressure}"),
          Text("${track[i].respirRate}")
        ]));
      }
...
<<<<<<< HEAD
                // RecordDeets(selected: selected, TxtTheme: TxtTheme),
                ...
                createTable(selected.patient.track),
              ]))),
...
class RecordDeets extends StatelessWidget {
  const RecordDeets({
    Key key,
                  ...
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) => Nurse(
                            edit: "Yes", index: i, pat: selected.patient)),
                  );
                },
              ),
...
>>>>>>> fd0661f913a81e9186a49429c6b692b1aab714db
                    ),
                    // child: ListView.builder(
                    //   itemCount: selected.patient.track.length,
                    //   itemBuilder: (context, i) {
